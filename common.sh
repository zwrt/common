...
cd ${GITHUB_WORKSPACE}
bash <(curl -fsSL https://raw.githubusercontent.com/zwrt/common/main/custom/first.sh)
}
...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/zwrt/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/zwrt/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/zwrt/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/zwrt/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
