...
cd ${GITHUB_WORKSPACE}
bash <(curl -fsSL https://raw.githubusercontent.com/skywrt/common/main/custom/first.sh)
}
...
function Diy_update() {
bash <(curl -fsSL https://raw.githubusercontent.com/skywrt/common/main/custom/ubuntu.sh)
if [[ $? -ne 0 ]];then
...
sed -i '/skywrt/d; /helloworld/d; /passwall/d; /OpenClash/d' "feeds.conf.default"
...
if [[ `grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config` -eq '1' ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/skywrt/common/main/jpg/${pmg}.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
  if [[ $? -ne 0 ]]; then
    echo "拉取文件错误,请检测网络"
    exit 1
  fi
...
if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
  ...
  wget -q https://github.com/skywrt/common/releases/download/API/AdGuardHome.api -O AdGuardHome.api
  if [[ $? -ne 0 ]];then
    curl -fsSL https://github.com/skywrt/common/releases/download/API/AdGuardHome.api -o AdGuardHome.api
  fi
...
if [[ -z "${amlogic_kernel}" ]]; then
  curl -fsSL https://github.com/skywrt/common/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
